...
HABILITAR_ZOOM = False # ZOOM DESACTIVADO: Se muestran los 10s íntegros
ZOOM_INICIO = 50
ZOOM_FIN = 150
...
vpre2_vec = h.Vector().record(n3(0.5)._ref_v) # Recuperamos Vpre2
vpost_vec = h.Vector().record(n2(0.5)._ref_v)
i1_vec = h.Vector().record(syn1._ref_i)
i2_vec = h.Vector().record(syn2._ref_i)       # Recuperamos i2
g1_vec = h.Vector().record(syn1._ref_g)
g2_vec = h.Vector().record(syn2._ref_g)       # Recuperamos g2
...
vpre2_neuron = np.array(vpre2_vec)[SLICE:]
vpost_neuron = np.array(vpost_vec)[SLICE:]

# Salida nativa de NEURON en nA
i1_neuron = -np.array(i1_vec)[SLICE:] 
i2_neuron = -np.array(i2_vec)[SLICE:] 

g1_neuron = np.array(g1_vec)[SLICE:]
g2_neuron = np.array(g2_vec)[SLICE:]

# =========================================================================
# PARTE 3: GRÁFICAS PDF MATPLOTLIB (Grid 3x2) - AMBAS SINAPSIS
# =========================================================================
...
axs[0, 0].plot(df_neun['Time'], df_neun['vpre2'], label=r'$V_{\mathrm{pre2}}$', color='green', alpha=0.8)
...
axs[1, 0].plot(df_neun['Time'], df_neun['i1'], label=r'$i_{1}$', color='lightcoral')
axs[1, 0].plot(df_neun['Time'], df_neun['i2'], label=r'$i_{2}$', color='limegreen')
axs[1, 0].set_ylabel(r'Corriente ($pA$)')
axs[1, 0].legend(loc='upper right')
axs[1, 0].grid(True, alpha=0.3)

axs[2, 0].plot(df_neun['Time'], df_neun['g1'], label=r'$g_{1}$', color='darkred')
axs[2, 0].plot(df_neun['Time'], df_neun['g2'], label=r'$g_{2}$', color='darkgreen')
axs[2, 0].set_ylabel(r'Cond. ($pS$)')
...
axs[0, 1].plot(t_neuron, vpre2_neuron, label=r'$V_{\mathrm{pre2}}$', color='green', alpha=0.8)
...
axs[1, 1].plot(t_neuron, i1_neuron, label=r'$i_{1}$', color='lightcoral')
axs[1, 1].plot(t_neuron, i2_neuron, label=r'$i_{2}$', color='limegreen')
axs[1, 1].set_ylabel(r'Corriente ($nA$)') 
axs[1, 1].legend(loc='upper right')
axs[1, 1].grid(True, alpha=0.3)

axs[2, 1].plot(t_neuron, g1_neuron, label=r'$g_{1}$', color='darkred')
axs[2, 1].plot(t_neuron, g2_neuron, label=r'$g_{2}$', color='darkgreen')
axs[2, 1].set_ylabel(r'Cond. ($pS$)')
...
# =========================================================================
# PARTE 4: GRÁFICAS HTML BOKEH (Grid 3x2) - AMBAS SINAPSIS
# =========================================================================
...
p_neun_v.line(df_neun['Time'], df_neun['vpre2'], legend_label="V_pre2", color="green", line_width=1.5)
...
p_neun_i.line(df_neun['Time'], df_neun['i1'], legend_label="i1", color="lightcoral", line_width=1.5)
p_neun_i.line(df_neun['Time'], df_neun['i2'], legend_label="i2", color="limegreen", line_width=1.5)
p_neun_i.yaxis.axis_label = "Corriente (pA)"

p_neun_g = figure(width=W, height=H, tools=TOOLS, x_range=p_neun_v.x_range)
p_neun_g.line(df_neun['Time'], df_neun['g1'], legend_label="g1", color="darkred", line_width=2)
p_neun_g.line(df_neun['Time'], df_neun['g2'], legend_label="g2", color="darkgreen", line_width=2)
p_neun_g.xaxis.axis_label = "Tiempo (ms)"
p_neun_g.yaxis.axis_label = "Cond. (pS)"

# --- NEURON ---
p_neuron_v = figure(**fig_kws_neuron)
p_neuron_v.line(t_neuron, vpre1_neuron, legend_label="V_pre1", color="red", line_width=1.5)
p_neuron_v.line(t_neuron, vpre2_neuron, legend_label="V_pre2", color="green", line_width=1.5)
...
p_neuron_i.line(t_neuron, i1_neuron, legend_label="i1", color="lightcoral", line_width=1.5)
p_neuron_i.line(t_neuron, i2_neuron, legend_label="i2", color="limegreen", line_width=1.5)
p_neuron_i.yaxis.axis_label = "Corriente (nA)"

p_neuron_g = figure(width=W, height=H, tools=TOOLS, x_range=p_neuron_v.x_range)
p_neuron_g.line(t_neuron, g1_neuron, legend_label="g1", color="darkred", line_width=2)
p_neuron_g.line(t_neuron, g2_neuron, legend_label="g2", color="darkgreen", line_width=2)
p_neuron_g.xaxis.axis_label = "Tiempo (ms)"
p_neuron_g.yaxis.axis_label = "Cond. (pS)"
